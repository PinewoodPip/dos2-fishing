...
local Fishing = {
    ...
    ABILITY_SCHOOL_FISH_PER_LEVEL = 5, -- Amount of fish that has to be caught per level in the Fishermancy ability.
    ABILITY_SCHOOL_FISH_PER_LEVEL_GROWTH = 6, -- Extra amount of fish that need to be caught per level in the Fishermancy ability, for each level after the first.
...
---Returns whether a position is near a fishable surface.
...
Ext.RegisterConsoleCommand("fishavailability", function (_)
    local availability = {} ---@type table<string, {Rarity: ItemLib_Rarity, Behaviour: Fishing.Fish.BehaviourType, Acts: table<string, integer>, Total: integer, Regions: table<string, string>}>
    for _,fish in pairs(Fishing.GetFishes()) do
        availability[fish.ID] = {
            Behaviour = fish.Behaviour,
            Acts = {},
            Total = 0,
            Regions = {},
        }
